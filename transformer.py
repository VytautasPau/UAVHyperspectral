...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, dim, depth, heads, mlp_dim,
                 pool='cls', channels=3, dim_head=64, dropout=0., emb_dropout=0., row=None):
        super().__init__()
        if row is not None:
        ...
        assert image_height % patch_height == 0 and image_width % patch_width == 0, f'Image dimensions {image_width} {image_height} must be divisible by the patch size {patch_width} {patch_height}. '
    ...
    def forward(self, img):
        ...
        b, n, _ = x.shape   # batch size, N = (H * W / P**2), _ = (P * P * C)

        cls_tokens = repeat(self.cls_token, '() n d -> b n d', b=b)  # repeat on batch
        x = torch.cat((cls_tokens, x), dim=1)  # append to start
