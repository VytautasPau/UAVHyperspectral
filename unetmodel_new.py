...
class UNet(nn.Module):
    def __init__(self, in_bands, out_class, col, row, pool_kernel_size=2, drop1=0.1, drop2=0.05):
        ...
        self.decoder2 = nn.Sequential(
            nn.Linear((col //2)**2, out_class),
            nn.LeakyReLU(),
        )
        self.decoder3 = nn.Sequential(
            nn.Linear((col //4)**2, out_class),
            nn.LeakyReLU(),
        )
        self.decoder5 = nn.Sequential(
            nn.Linear((col //8)**2, out_class),
            nn.LeakyReLU(),
        )
        #self.flat = int(col // 8 ) * int(row // 8) + int(col // 4) * int(row // 4) + int(col // 2) * int(row // 2)
        self.flat = self.oc2 + self.oc3 + self.oc3 // 2
        self.gn = GaussianNoise(sigma=0.1, is_relative_detach=True)
        self.maxpool = nn.MaxPool1d(2)
        self.decoder = nn.Sequential(
            nn.Linear(self.flat, in_bands),
            # nn.ReLU(),
            nn.Sigmoid(),
        )
    ...
    def forward(self, x):
        ...
        endm2 = torch.flatten(x2, start_dim=2)
        endm2 = self.decoder2(endm2)
        endm3 = torch.flatten(x3, start_dim=2)
        endm3 = self.decoder3(endm3)
        endm5 = torch.flatten(x5, start_dim=2)
        endm5 = self.decoder5(endm5)

        endm = torch.cat((endm2, endm3, endm5), dim=1).transpose(1, 2)
        endm1 = self.gn(endm)
        #endm = self.maxpool(endm)
        #endm = endm.permute(0, 2, 1)
        endm1 = self.decoder(endm1).squeeze()
        ...
        mult = torch.matmul(abd, endm1)
        mult = mult.permute(0, 3, 1, 2)

        return abd, mult, endm1
